<?php
  ...
  if(isset($_SESSION["pseudo"])){header('Location: http://safetyglass-app.com/gestion/acceuil.php');}

  $sql='SELECT userName,idUser FROM DB_Pyrobel.user';

  $listUser=$db->query($sql);

  if($_POST) //On check le mot de passe
  {
      $query=$db->prepare('SELECT iduser, userName, password,nivdroit
          FROM DB_Pyrobel.user WHERE userName = :pseudo');
      ...
      if ($data['password'] == $_POST['password']) { ///Acces OK !
          ...
          header('Location: http://safetyglass-app.com/gestion/acceuil.php');
          exit();
